...
from .metrics import compute_pattern_expression, estimate_channel_covariance, fit_ridge
from .prepare import EncodingDesign, build_design
    ...
    return SubjectEncoding(
        tables={
            ...
            "pattern_expression": pd.concat(
                ...
                ignore_index=True,
            ).sort_values(["subject", "trial", "component", "time"]).reset_index(drop=True),
...
def _fit_fold(
    *,
    ...
    for time_index, time in enumerate(times):
        residuals = (
            data[train_rows, :, time_index]
            - train_design @ beta[:, :, time_index]
        )
    ...
    *,
    ...
    for component_index, component in enumerate(components):
        tables.append(
            pd.DataFrame(
                {
                    "subject": np.repeat(subject, len(rows) * len(times)),
                    "trial": np.repeat(trial_ids[rows], len(times)),
    ...
    *,
    ...
    for predictor_index, predictor in enumerate(predictor_names):
        tables.append(
            pd.DataFrame(
                {
                    "subject": np.repeat(subject, channel_count * len(times)),
