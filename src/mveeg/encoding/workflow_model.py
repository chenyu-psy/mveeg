...
def _fit_one_encoding_subject(
    *,
    ...
    source_to_condition: dict[str, str],
    train_condition_labels: tuple[str, ...] | None,
    formula_condition_encoding: pd.DataFrame,
    effective_design_cfg: EncodingConfig,
    formula_predictors: list[str],
    formula_add_intercept: bool,
    cv_n_splits: int,
    cv_shuffle: bool,
    cv_random_state: int,
    time_window_ms: int,
    ...
    """Fit and save one subject's encoding model result.
    ...
    source_to_condition : dict[str, str]
        Mapping from raw labels to analysis condition labels.
    train_condition_labels : tuple[str, ...] | None
        Optional analysis labels allowed in model-training folds.
    formula_condition_encoding : pd.DataFrame
        Condition-to-predictor design table restricted to formula predictors.
    effective_design_cfg : EncodingConfig
        Design validation settings matching the parsed formula.
    formula_predictors : list[str]
        Predictor names used for training strength and long testing tables.
    formula_add_intercept : bool
    ...
    standardize_data : bool
        Whether to standardize held-out data using training-fold statistics.
    n_null_repeats : int
        Number of condition-shuffled null draws per fold.
    progress_bar : object
        Per-subject tqdm progress bar. Encoding updates once after the observed
        fit for each fold and once for each null draw.

    Returns
    -------
    dict[str, np.ndarray]
        Saved subject-level encoding model payload.
    """
    ...
    for fold_ix, (train_idx, test_idx) in enumerate(
        ...
        if len(train_model_idx) == 0:
            raise ValueError("No training trials remain after applying train_condition_labels.")
        ...
        for predictor_ix, predictor_name in enumerate(predictor_names):
            raw_beta_patterns[fold_ix, predictor_ix, :, :] = fold_patterns[predictor_name]

        for effect_ix, effect_name in enumerate(effect_predictors):
            observed_strength = compute_pattern_strength(fold_patterns[effect_name])
            pattern_strength_pattern[fold_ix, effect_ix, :] = observed_strength
        progress_bar.update(1)

        for null_ix in range(n_null_repeats):
            progress_bar.set_postfix_str(
                f"fold {fold_id}/{cv_n_splits}, null {null_ix + 1}/{n_null_repeats}"
            )
            ...
            for effect_ix, effect_name in enumerate(effect_predictors):
                null_strength = compute_pattern_strength(shuffled_patterns[effect_name])
                pattern_strength_null_draws[fold_ix, effect_ix, null_ix, :] = null_strength
            progress_bar.update(1)

        progress_bar.set_postfix_str(f"fold {fold_id}/{cv_n_splits}, testing")
        ...
        coefficient_wide_df, _ = build_testing_coefficient_tables(
            subject=subject_id,
            fold_id=fold_id,
    ...
    coef_values = np.column_stack(
        [
            testing_coefficient_wide_df[f"coef_{predictor_name}"].to_numpy(dtype=float)
    ...
    payload = {
        ...
        "source_condition_keys": np.asarray(sorted(source_to_condition.keys()), dtype=object),
        "source_condition_values": np.asarray(
            [source_to_condition[key] for key in sorted(source_to_condition.keys())],
            dtype=object,
        ),
        ...
        "coef_condition": testing_coefficient_wide_df["condition"].to_numpy(dtype=object),
...
def run_encoding_workflow(
    *,
    ...
    def process_one_subject(subject_id: str, progress_bar):
        ...
        if saved is None:
            saved = _fit_one_encoding_subject(
                subject_id=subject_id,
                ...
                source_to_condition=source_to_condition,
                train_condition_labels=train_condition_labels,
                formula_condition_encoding=formula_condition_encoding,
                effective_design_cfg=effective_design_cfg,
                formula_predictors=formula_predictors,
                formula_add_intercept=formula_add_intercept,
        ...
        subject_payloads[str(saved["subject"].item())] = saved
        return saved, used_saved_result

    skipped_subjects_df = process_subjects(
        subject_ids=[str(subject_id) for subject_id in subject_ids],
        progress_total=cv_n_splits * (n_null_repeats + 1),
    ...
    testing_tables = []
    testing_wide_tables = []
    for saved in subject_payloads.values():
    ...
    subject_summary_df = (
        pd.DataFrame(subject_summary_rows)
        .sort_values("subject")
        .reset_index(drop=True)
    )
