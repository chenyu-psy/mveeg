...
from ._analysis import SubjectEncoding, encode_subject
from ._prepare import DEFAULT_DROP_TYPES, select_trials, validate_groups
...
class EncodingPipeline:
    ...
    def setup_cv(
        ...
        *,
        ...
        if seed is not None and (not isinstance(seed, (int, np.integer)) or int(seed) < 0):
    ...
    def _refresh_dataset(self) -> None:
        ...
        if self.stage not in {"prepared", "preprocessed"}:
            raise ValueError(
                f"Encoding requires a prepared or preprocessed dataset, found stage {self.stage!r}."
            )
...
def _prepare_subject(
    ...
    *,
    ...
    leading = ["subject_index", "epoch_index", "training_group", "expression_group"]
...
def _name_coefficient_channels(encoded: SubjectEncoding, channels: pd.DataFrame) -> None:
    ...
    encoded.tables["coefficients"] = table[
        ["subject_index", "time", "channel", "predictor", "beta"]
    ]
