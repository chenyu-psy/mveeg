...
from ..._dataset.store import DatasetBuilder, fingerprint_epochs
from ..._provenance import fingerprint
from .. import events as event_engine
from .. import steps
...
class ExternalPipeline:
    ...
    def __init__(self, *, subject_index: str | int, data: np.ndarray | mne.BaseEpochs):
        ...
        self.subject_index = normalize_subject_index(subject_index)
        ...
        if isinstance(data, mne.BaseEpochs):
            ...
            if array.ndim != 3:
                raise ValueError("External array data must have shape (epochs, channels, times).")
    ...
    def make_epochs(
        ...
        *,
        ...
        sampling_rate = event_engine._normalize_sampling_rate(sampling_rate)
    ...
    def transform_metadata(
        ...
        **variables: Callable[[pd.DataFrame], object],
        ...
        """Immediately evaluate ordered scalar or trial-aligned variables."""
        validate_metadata_variables(variables)
        epochs = self._epochs_with_identity()
    ...
    def select_epochs(
        ...
        *,
        ...
        self._epochs = steps.select_epochs(self._require_epochs(), include=include, exclude=exclude)
        self._history.append(
            {
                "step": "select_epochs",
    ...
    def _epochs_with_identity(self) -> mne.Epochs:
        """Attach identity once, then validate it after eager transformations."""
        return steps._ensure_identity(self._require_epochs(), self.subject_index)
