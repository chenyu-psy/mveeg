...
from .._dataset.metadata import IDENTITY_COLUMNS, normalize_identity
from .events import (
    _event_metadata,
    _find_trial_matches,
    _normalize_events,
    _normalize_sampling_rate,
...
def make_epochs(
    ...
    **kwargs
    ...
    raw_events = (
        extract_events(raw, event_id=event_id) if events is None else _normalize_events(events)
    )
...
def sync_eyelink(
    ...
    *,
    ...
    if baseline is not None:
        ...
        if len(picks):
            rescale(
                eye_epochs.get_data(copy=False),
                eye_epochs.times,
                baseline,
                copy=False,
                picks=picks,
...
def align_behavior(epochs: mne.Epochs, behavior: pd.DataFrame) -> mne.Epochs:
    ...
    reserved = sorted(set(IDENTITY_COLUMNS).intersection(behavior.columns))
    if reserved:
        raise ValueError(f"Behavior cannot define reserved identity columns: {reserved}.")
    ...
    metadata = pd.concat([base.reset_index(drop=True), behavior.reset_index(drop=True)], axis=1)
...
def assign_identity(epochs: mne.Epochs, subject_index: str | int) -> mne.Epochs:
    ...
    if existing:
        raise ValueError(
            f"Identity columns are generated at build time and already exist: {existing}."
        )
...
def _ensure_identity(epochs: mne.Epochs, subject_index: str | int) -> mne.Epochs:
    """Attach or validate generated identity and restore sequential epoch indices."""

    metadata = _metadata(epochs)
    present = set(IDENTITY_COLUMNS).intersection(metadata)
    if not present:
        return assign_identity(epochs, subject_index)
    if present != set(IDENTITY_COLUMNS):
        raise ValueError("Epoch metadata contains an incomplete identity pair.")
    normalized = normalize_identity(metadata, context="epoch metadata")
    expected_subject = str(subject_index)
    if not normalized["subject_index"].eq(expected_subject).all():
        raise ValueError("Epoch metadata identity does not match subject_index.")
    normalized["epoch_index"] = np.arange(len(normalized), dtype=int)
    return _with_metadata(epochs, normalized)
