...
from .models import haufe_patterns, make_classifier, native_evidence, pattern_components
from .prepare import average_training_trials, sample_balanced
...
def decode_subject(
    *,
    ...
    if any(
        ...
        for values in (evidence, generalization, conditions)
    ):
    ...
    if any(count < folds for count in counts.values()):
        raise ValueError(
            f"Every class needs at least folds={folds} trials; counts were {counts}."
        )
    ...
    if effective_jobs == 1:
        repeat_results = (
            _decode_repeat(repeat=repeat, **shared)
        ...
        repeat_results = Parallel(
            ...
            delayed(_decode_repeat)(repeat=repeat, **shared)
            for repeat in range(1, repeats + 1)
        )
    ...
    for result in tqdm(
        ...
        if output == "all":
            ...
            mean_results[result.repeat] = _summarize_repeat(result.tables)
    ...
    if output == "all":
        tables = _combine_all(all_results)
    else:
        tables = _combine_mean(mean_results)
...
def _decode_repeat(
    *,
    ...
    for fold_index, (train_index, test_index) in enumerate(splits):
        ...
        condition_rows = [
            (condition, rows) for condition, rows in condition_rows if len(rows)
        ]
...
def _combine_all(
    ...
    return {
        "accuracy": tables["accuracy"][
            ["subject", "repeat", "fold", "time", "permutation", "accuracy", "n_correct", "n_trials"]
        ],
        "classifier_evidence": tables["classifier_evidence"][
            ["subject", "trial", "repeat", "fold", "time", "evidence"]
        ],
        "confusion_matrix": tables["confusion_matrix"][
            ["subject", "repeat", "fold", "time", "actual", "predicted", "count"]
        ],
        "patterns": tables["patterns"][
            ["subject", "repeat", "fold", "time", "channel_index", "component", "pattern"]
        ],
        **(
            {
...
def _summarize_repeat(
    ...
    confusion_keys = ["subject", "repeat", "time", "actual", "predicted"]
    pattern_keys = ["subject", "repeat", "time", "channel_index", "component"]
    summarized = {
        "accuracy": _summarize_repeat_accuracy(tables["accuracy"], ["time"]),
        "classifier_evidence": _summarize_repeat_evidence(
            tables["classifier_evidence"]
        ),
        "confusion_matrix": (
...
def _summarize_repeat_accuracy(
    ...
    keys = ["subject", "repeat", *time_columns, "permutation"]
    summarized = table.groupby(keys, as_index=False, sort=False)[
        ["n_correct", "n_trials"]
    ].sum()
...
def _summarize_repeat_evidence(table: pd.DataFrame) -> pd.DataFrame:
    rows = []
    keys = ["subject", "repeat", "trial", "time"]
...
def _combine_mean(
    ...
    confusion_keys = ["subject", "time", "actual", "predicted"]
    pattern_keys = ["subject", "time", "channel_index", "component"]
    patterns = (
        tables["patterns"]
        .groupby(pattern_keys, as_index=False, sort=True)[
            ["pattern_sum", "n_models"]
        ]
        .sum()
    )
    ...
    formatted = {
        ...
        "classifier_evidence": _combine_mean_evidence(
            tables["classifier_evidence"]
        ),
        "confusion_matrix": (
            tables["confusion_matrix"]
...
def _combine_mean_accuracy(
    table: pd.DataFrame,
    time_columns: list[str],
) -> pd.DataFrame:
    keys = ["subject", *time_columns, "permutation"]
    counts = table.groupby(keys, as_index=False, sort=True)[
        ["n_correct", "n_trials"]
    ].sum()
    ...
    for condition, rows in condition_rows:
        ...
        for test_time_index, test_time in enumerate(times):
            predicted = model.predict(
                scaler.transform(data[rows, :, test_time_index])
            )
            output.append(
                _generalization_row(
                    subject,
...
def _valid_permutation(
    ...
    for _ in range(1000):
        ...
        if all(
            ...
            return shuffled
    raise ValueError("Could not draw a permutation with every class represented in each training fold.")
