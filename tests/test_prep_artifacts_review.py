...
def _make_review_epochs(n_epochs=3):
    ...
    metadata = pd.DataFrame(
        {"subject_index": ["4001"] * n_epochs, "epoch_index": range(n_epochs)}
    )
...
def test_matplotlib_review_visuals_and_direct_controls():
    ...
    assert browser.axes.get_position().bounds == pytest.approx(
        (0.08, 0.10, 0.915, 0.75)
    )
    assert browser.axes.get_xlim() == pytest.approx(
        (0, 2 * browser._samples_per_epoch)
    )
    ...
    black = [
        ...
        if to_hex(line.get_color()) == "#000000"
    ]
    ...
    key_callbacks = browser.figure.canvas.callbacks.callbacks.get(
        "key_press_event", {}
    )
    ...
    browser._on_click(
        SimpleNamespace(inaxes=browser.axes, xdata=first_center, button=1)
    )
    assert session.get_status(0) == "accepted"
    assert any(
        text.get_text() == "Trial 0\nstimulus\naccepted"
    ...
    time_lock_lines = [
        line for line in browser.axes.lines if line.get_label() == "time-lock event"
    ]
...
def test_review_close_releases_preloaded_epochs_and_gui_resources():
    ...
    figure.canvas.callbacks.process(
        "close_event", CloseEvent("close_event", figure.canvas)
    )
...
def test_review_uses_02_epoch_guides_and_top_trial_labels():
    ...
    boundaries = [
        ...
        for line in browser.axes.lines
        if to_hex(line.get_color()) == "#000000"
        and line.get_linewidth() == 3
    ]
    time_lock_lines = [
        line
        for line in browser.axes.lines
        if to_hex(line.get_color()) == "#ff00ff"
    ]
    trial_labels = [
        text.get_text()
        for text in browser.axes.texts
...
def test_review_open_prints_all_group_counts(
    group_by, label, conditions, expected, capsys
):
