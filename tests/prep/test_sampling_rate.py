...
from mveeg.prep import init_external, open_pipeline
...
def test_external_array_accepts_only_sampling_rate_keyword():
    external = init_external(subject_index="4001", data=np.zeros((2, 1, 10)))
...
def test_manifest_requires_sampling_rate_column(tmp_path):
    dataset = (
    ...
    legacy = pd.read_csv(manifest_path, sep="\t").rename(columns={"sampling_rate": "sfreq"})
