...
from mveeg.prep import init_external
...
from mveeg.prep.quality.state import load_quality_state
...
def _prepared_dataset(tmp_path):
    ...
    return external.make_epochs(sampling_rate=100, ch_names=["Cz", "Pz"], tmin=-0.2).build_epochs(
        tmp_path / "prepared", task="task"
    )
...
def test_preprocess_label_relabel_and_dataset_review_method(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("mveeg.prep.review.figure.open_review_figure", fake_open)
...
def test_autoreject_channel_reasons_do_not_change_authoritative_status(tmp_path):
    ...
    assert table.loc[0, "channel_Pz"] == ("autoreject_bad_channel;autoreject_interpolated")
...
def test_missing_only_gaze_rule_does_not_require_geometry(tmp_path):
    ...
    state, _ = load_quality_state(quality_state_path(preprocessed.root, "4001", "task"))
...
def test_recompute_never_does_not_replace_persisted_gaze_geometry(tmp_path):
    ...
    assert reused.run_summary.to_dict("records") == [{"subject_index": "4001", "status": "reused"}]
    assert persisted["pipeline"]["gaze_geometry"] == GAZE_GEOMETRY
    assert "gaze_geometry" not in persisted


def test_preprocess_requires_current_gaze_geometry(tmp_path):
    prepared = _prepared_dataset(tmp_path)
