...
from mveeg.prep.quality.rules import _hf_window_starts
...
def _eligibility_state(
    ...
    return EligibilityResult(
        ...
        rule_masks={},
        channel_reasons=np.full((len(epochs), len(epochs.ch_names)), "", dtype="<U256"),
    )
...
def _label_rules(
    ...
    *,
    ...
    return label_artifact_rules(
        ...
        review_config=(_quiet_review_config() if review_config is None else review_config),
...
def _gaze_epochs() -> mne.Epochs:
    ...
    events = np.column_stack((np.arange(3) * 200, np.zeros(3, dtype=int), np.ones(3, dtype=int)))
    return mne.EpochsArray(
        data,
        info,
        events=events,
...
def test_quality_state_round_trip_uses_no_object_arrays(tmp_path):
    ...
    assert loaded_autoreject["schema_version"] == 3
...
def test_autoreject_accepts_scalar_candidates_and_shuffled_cv(monkeypatch, tmp_path):
    ...
    class FakeAutoReject:
        ...
        def fit_transform(self, epochs, *, return_log):
            ...
            self.loss_ = {
                "eeg": np.zeros((len(self.consensus), len(self.n_interpolate), 5), dtype=float)
            }
    ...
    monkeypatch.setattr("mveeg.prep.quality.autoreject.AutoReject", FakeAutoReject)
    epochs = _epochs()
    eligibility = check_eligibility(epochs, _eligibility_config())
...
def test_quality_state_requires_current_schema(tmp_path):
    """Pre-0.3 sidecars are rejected instead of silently upgraded."""
    ...
    with pytest.raises(KeyError, match="quality_schema_version"):
...
def test_actual_autoreject_keeps_full_order_and_stores_only_eeg_labels():
    ...
    events = np.column_stack((np.arange(20) * 100, np.zeros(20, dtype=int), np.ones(20, dtype=int)))
...
def test_hf_reference_exclusions(
    ...
    epochs = _hf_epochs_from_log_power(-24 + np.asarray([[0.0], [2.0], [20.0], [12.0]]))
    eligible = np.ones(len(epochs), dtype=bool)
    bad_epochs = np.zeros(len(epochs), dtype=bool)
    ...
    assert ("high_frequency_noise" in result.rejected_reasons[3, 0]) is target_is_noisy


def test_hf_info_bads_are_not_scored_but_ignored_channels_keep_reason(
    no_op_epoch_filter,
):
    epochs = _hf_epochs_from_log_power(np.asarray([[-24.0, -24.0], [-22.0, -22.0], [-12.0, -12.0]]))
    epochs.info["bads"] = ["E1"]
    result = _label_rules(
        epochs,
...
def test_hf_config_rejects_unknown_mad_multiplier(no_op_epoch_filter):
...
def test_gaze_rules_require_eyegaze_channels_and_reject_stage_is_unsupported():
    ...
    with pytest.raises(ValueError, match="reject.gaze is unsupported"):
        label_artifact_rules(
            ...
            autoreject_labels=np.full((len(gaze_epochs), 1), -1, dtype=np.int8),
