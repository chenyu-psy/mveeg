...
from mveeg._dataset.manifest import MANIFEST_COLUMNS
from mveeg._dataset.store import DatasetBuilder
from mveeg._provenance import fingerprint
from mveeg.prep import DatasetPipeline, init_external, open_pipeline
from mveeg.prep.events import _ordered_code_alignment, extract_events
from mveeg.prep.eyelink import (
    eyelink_files as _eyelink_files,
)
from mveeg.prep.eyelink import (
...
def _write_raw_subject(root, subject: str, n_events: int) -> None:
    ...
    raw = mne.io.RawArray(np.zeros((1, 600)), mne.create_info(["Cz"], 100, "eeg"), verbose="ERROR")
    raw.set_annotations(
        mne.Annotations(
...
def test_eyelink_registration_preserves_canonical_provenance(tmp_path) -> None:
    ...
    assert pipeline._pipeline_spec()["load_eyelink"] == {"format": "asc_or_edf"}
...
def test_eyelink_edf_is_converted_before_input_fingerprinting(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("mveeg.prep.eyelink.shutil.which", lambda name: "/bin/edf2asc")
    ...
    monkeypatch.setattr("mveeg.prep.eyelink.subprocess.run", convert)
...
def test_eyelink_conversion_reports_missing_source_and_converter(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("mveeg.prep.eyelink.shutil.which", lambda name: None)
...
def test_eyelink_conversion_failure_reports_process_output(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("mveeg.prep.eyelink.shutil.which", lambda name: "/bin/edf2asc")
    monkeypatch.setattr(
        "mveeg.prep.eyelink.subprocess.run",
...
def test_eyelink_sync_removes_leading_warmup_trials() -> None:
    ...
    eeg_raw.set_annotations(mne.Annotations([2.0, 4.0], [0.0, 0.0], ["condition_a", "condition_b"]))
    eeg = mne.Epochs(
        eeg_raw,
...
def test_event_extraction_restricts_numeric_messages_to_configured_codes() -> None:
    ...
    raw = mne.io.RawArray(np.zeros((1, 300)), mne.create_info(["Cz"], 100, "eeg"), verbose="ERROR")
    raw.set_annotations(
        mne.Annotations(
            [1.0, 2.0],
            [0.0, 0.0],
...
def test_external_merge_and_transform_protect_identity() -> None:
    """External metadata merges are one-to-one and identity stays reserved."""
    epochs = _epochs(3)
    ...
    with pytest.raises(ValueError, match="cannot replace"):
        init_external(subject_index="4001", data=merged).transform_metadata(
            epoch_index=lambda frame: range(len(frame))
        )
...
def test_external_pipeline_writes_standard_dataset(tmp_path) -> None:
    ...
    pipeline = init_external(subject_index="4001", data=np.zeros((3, 2, 20), dtype=float))
    dataset = (
        pipeline.make_epochs(sampling_rate=100, ch_names=["Cz", "Pz"])
        .merge_metadata(metadata)
        .transform_metadata(
            centered=lambda frame: frame["score"] - frame["score"].mean(),
        )
    ...
    assert (
        dataset.path_for_subject("4001", "artifacts").name
        == "sub-4001_task-exp4_desc-artifacts.tsv"
    )
...
def test_raw_gaze_geometry_is_persisted_after_build_without_changing_fingerprint(
    ...
    assert (
        configured_dataset.manifest.loc[0, "pipeline_fingerprint"]
...
def test_prepared_gaze_geometry_is_idempotent_overwritable_and_inherited(
    ...
    import mveeg.prep.pipeline.dataset as dataset_module
    ...
    with monkeypatch.context() as patch:
        writes = []
        original_write = dataset_module.write_json_atomic

        def track_write(value, path):
            writes.append(path)
            original_write(value, path)

        patch.setattr(dataset_module, "write_json_atomic", track_write)
        assert (
...
def test_preprocessed_dataset_cannot_configure_gaze(tmp_path) -> None:
    ...
    preprocessed = DatasetPipeline(tmp_path / "preprocessed", run_summary=summary)
...
def test_raw_changed_uses_actual_transformed_metadata_values(tmp_path) -> None:
    input_dir = tmp_path / "raw"
    _write_raw_subject(input_dir, "4001", 2)
    output = tmp_path / "prepared"

    first = init_pipeline(input_dir)
    first.load_eeg("*_raw.fif").make_epochs(
        event_id={"stim": 1}, time_window=(0, 0.1)
    ).transform_metadata(derived=lambda frame: 1)
    first.build_epochs(output, task="task")

    changed = init_pipeline(input_dir)
    changed.load_eeg("*_raw.fif").make_epochs(
        event_id={"stim": 1}, time_window=(0, 0.1)
    ).transform_metadata(derived=lambda frame: 2)
    rebuilt = changed.build_epochs(output, task="task", recompute="changed")

    assert rebuilt.run_summary.loc[0, "status"] == "written"
    assert rebuilt.load_epochs("4001").metadata["derived"].tolist() == [2, 2]
