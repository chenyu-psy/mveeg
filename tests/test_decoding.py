...
from mveeg.prep.dataset import DatasetBuilder, open_pipeline
...
def test_multiclass_all_output_preserves_native_geometry_and_cv_rows():
    ...
    evidence_only = result.tables["classifier_evidence"].query("trial == 12")
    assert len(evidence_only) == 2 * 2  # folds x time bins
    class_trial = result.tables["classifier_evidence"].query("trial == 0")
    ...
    assert set(result.tables["generalization"]["condition"]) == {
    ...
    assert result.tables["classifier_evidence"].query("trial == 13").empty
...
def test_binary_mean_output_keeps_scalar_evidence_and_weighted_accuracy():
    ...
    assert accuracy.columns.tolist() == [
        "subject", "time", "permutation", "accuracy", "n_correct", "n_trials"
    ]
...
def test_incremental_mean_matches_fold_level_results():
    ...
    repeat_accuracy = detailed["accuracy"].groupby(
        ["subject", "repeat", "time", "permutation"], as_index=False
    )[["n_correct", "n_trials"]].sum()
    repeat_accuracy["accuracy"] = (
        repeat_accuracy["n_correct"] / repeat_accuracy["n_trials"]
    )
    expected_accuracy = repeat_accuracy.groupby(
        ["subject", "time", "permutation"], as_index=False
    )["accuracy"].mean()
    np.testing.assert_allclose(mean["accuracy"]["accuracy"], expected_accuracy["accuracy"])

    detailed_evidence = detailed["classifier_evidence"].query(
        "trial == 12 and time == 25"
    )
    expected_evidence = np.mean(
        np.stack(detailed_evidence["evidence"].map(np.asarray)), axis=0
    )
    observed_evidence = np.asarray(
        mean["classifier_evidence"].query("trial == 12 and time == 25")[
            "evidence"
        ].iloc[0]
    )
    ...
    expected_confusion = detailed["confusion_matrix"].groupby(
        ["subject", "time", "actual", "predicted"], as_index=False
    )["count"].sum()
    pd.testing.assert_frame_equal(mean["confusion_matrix"], expected_confusion)

    expected_patterns = detailed["patterns"].groupby(
        ["subject", "time", "channel_index", "component"], as_index=False
    )["pattern"].mean()
    pd.testing.assert_frame_equal(mean["patterns"], expected_patterns)

    expected_generalization = detailed["generalization"].groupby(
        ["subject", "condition", "train_time", "test_time", "permutation"],
        as_index=False,
    )[["n_correct", "n_trials"]].sum()
...
def test_trial_selection_drops_only_a_redundant_subject_alias():
    ...
    metadata["subject"] = metadata["subject_index"].astype(int)
    ...
    with pytest.raises(ValueError, match="reserved decoding columns"):
        select_trials(
            metadata,
            target="condition",
            classes={"a": ["a"], "b": ["b"]},
...
def test_generalization_mapping_selects_conditions_independently():
    ...
    selected, class_labels, evidence_labels, generalization_labels, rows = (
        select_trials(
            metadata,
            target="condition",
            classes=classes,
...
def test_pipeline_writes_public_duckdb_and_reuses_completed_subjects(tmp_path):
    ...
    with duckdb.connect(str(result_file), read_only=True) as connection:
        ...
        assert evidence_schema.loc[
            evidence_schema["column_name"].eq("evidence"), "column_type"
        ].item() == "DOUBLE[]"
        ...
        assert connection.execute(
            ...
            "JOIN trials t USING (subject, trial) WHERE t.condition='x'"
        ).fetchone()[0] == 0
        assert connection.execute(
        ...
        first_accuracy = connection.execute(
            "SELECT * FROM accuracy WHERE subject='001' ORDER BY time, permutation"
        ).df()
    ...
    with duckdb.connect(str(result_file), read_only=True) as connection:
        assert connection.execute(
            "SELECT subject, status FROM subjects ORDER BY subject"
...
def test_generated_seed_is_reused_and_changed_inputs_are_recomputed(tmp_path):
    ...
    with duckdb.connect(str(result_file), read_only=True) as connection:
        ...
        status, second_fingerprint = connection.execute(
            "SELECT status, fingerprint FROM subjects WHERE subject='001'"
        ).fetchone()
...
def test_failed_subject_is_recorded_without_partial_results(tmp_path):
    ...
    with duckdb.connect(str(result_file), read_only=True) as connection:
        status, reason = connection.execute(
            "SELECT status, reason FROM subjects WHERE subject='001'"
        ).fetchone()
        tables = {
            row[0]
            for row in connection.execute(
...
def _epochs(subject: str) -> mne.EpochsArray:
    ...
    return mne.EpochsArray(
        ...
        events=np.column_stack(
            [np.arange(len(labels)) * 20, np.zeros(len(labels), dtype=int), np.ones(len(labels), dtype=int)]
        ),
        event_id={"trial": 1},
        tmin=0,
        metadata=metadata,
