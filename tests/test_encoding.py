...
from mveeg.encoding.analysis import encode_subject
from mveeg.encoding.metrics import (
    compute_pattern_expression,
    estimate_channel_covariance,
    fit_ridge,
)
from mveeg.encoding.prepare import build_design, parse_formula, validate_groups
...
def test_default_penalties_recover_a_b_and_combined_expression():
    ...
    assert result.tables["pattern_expression"].groupby(
        ["trial", "component"]
    ).size().eq(1).all()
...
def test_heldout_trial_cannot_change_its_fold_model():
    ...
    heldout_fold = int(first.loc[first["trial"].eq(0), "fold"].iloc[0])

    changed = data.copy()
    changed[0] += 100
    second = encode_subject(data=changed, **settings).tables["pattern_expression"]
    same_fold = first["fold"].eq(heldout_fold) & first["trial"].ne(0)
    pd.testing.assert_frame_equal(
        first.loc[same_fold].reset_index(drop=True),
        second.loc[same_fold].reset_index(drop=True),
    )
    assert not np.allclose(
        first.loc[first["trial"].eq(0), "expression"],
        second.loc[second["trial"].eq(0), "expression"],
    )
...
def test_pipeline_writes_public_duckdb_and_condition_view(tmp_path):
    ...
    with duckdb.connect(str(result_file), read_only=True) as connection:
        ...
        n_expression = connection.execute(
            "SELECT COUNT(*) FROM pattern_expression"
        ).fetchone()[0]
        ...
        assert connection.execute(
            "SELECT MAX(n_trials) FROM condition_pattern_expression "
            "WHERE condition='probe'"
        ).fetchone()[0] == 3
        config = json.loads(
...
def test_pipeline_incrementally_adds_subject_and_rejects_config_change(tmp_path):
    ...
    with duckdb.connect(str(result_file), read_only=True) as connection:
        original = connection.execute(
            "SELECT * FROM coefficients WHERE subject='001' ORDER BY ALL"
        ).df()
